...
int main(int argc, char** argv){
	...
	for (int arg = 1; arg < argc; arg++) {
		if (argv[arg][0] == '-') {
			if (argv[arg][1] == 'h') {
				printf("\nUsage: ./Test [-h] [-n <num_test>] [-d <DBG>] [-c <num_coeffs_DBG>] [-M <paral_coeff>] [-y <max_cycles>]  \n\n");
				printf("\nANALISIS TEMPORAL : \n");
				...
				printf("DBG == 11: SOLAMENTE ANALISIS DE LA SEMILLA. Genera el archivo r.txt \n");
				return PYNQ_SUCCESS;
			}
	...
	printf(" ACELERACION DEC:  %f \n \n",	(float)tiempo_total_dec / (float)tiempo_total_dec_hw);
	printf(" FALLOS_ENC_SW: %d \n", fallos_enc);
	printf(" FALLOS_ENC_HW: %d \n", fallos_enc_HW);
	printf(" FALLOS_CRYPTO_SW: %d \n", fallos_crypto);
	printf(" FALLOS_CRYPTO_HW: %d \n", fallos_crypto_HW);
